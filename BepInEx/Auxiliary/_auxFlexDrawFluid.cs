    ...
    public class _auxFlexDrawFluid : MonoBehaviour
    {
        ...
        void Create()
        {
            ...
            if (m_scene && m_scene.container && m_scene.container.handle)
            {
                ...
                //m_prepareFluidMaterial = new Material(Nexus.BundleLoader.BundleLoaderPlugin.Instance.GetAssetBundle("flexshaders").LoadAllAssets()[5] as Shader);
                m_prepareFluidMaterial = Nexus.BundleLoader.BundleLoaderPlugin.Instance.GetAssetBundle("flexmaterials").LoadAllAssets()[6] as Material;
